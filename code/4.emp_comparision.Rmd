...
python analysis/source_tracker/GetV4Region.py -i data/processed/biom_tables/raw/all_w_taxonomy_table.biom_seqs/dna-sequences.fasta -l 90 -s > data/processed/emp_data//90bp_concrete_seqs.fasta
```
...
library(tidyverse)
library(qiime2R)
x90bp <- read_delim("data/processed/emp_data//90bp_concrete_seqs.fasta", ">", escape_double = FALSE, col_names = FALSE, trim_ws = TRUE)
...
#################
qiime tools import \
  --input-path data/processed/emp_data//90bp_concrete_seqs.fasta \
  --output-path data/processed/emp_data/90bp_concrete_seqs.qza \
...
qiime feature-table merge \
  ...
  --i-tables data/processed/emp_data/tapwater_table.qza \
...
qiime feature-table merge-seqs \
  --i-data data/processed/emp_data/tapwater_seqs.qza \
  --i-data data/processed/emp_data/90bp_concrete_seqs.qza \
...
python analysis/source_tracker/GetV4Region.py -i data/processed/biom_tables/decontaminated/all_w_taxonomy_decontam_table.biom_seqs/dna-sequences.fasta -l 90 -s > data/processed/emp_data/90bp_decontam_concrete_seqs.fasta
```
...
library(tidyverse)
library(qiime2R)
x90bp <- read_delim("data/processed/emp_data/90bp_decontam_concrete_seqs.fasta", ">", escape_double = FALSE, col_names = FALSE, trim_ws = TRUE)
x90bp <- data.frame(lapply(x90bp, na.omit)) %>% mutate('#OTU ID' = X1, q2_feature_id = X2) %>% select('#OTU ID',q2_feature_id)
...
qiime tools import \
  --input-path data/processed/emp_data/90bp_decontam_concrete_seqs.fasta \
  --output-path data/processed/emp_data/90bp_decontam_concrete_seqs.qza \
  --type 'FeatureData[Sequence]'
...
qiime feature-table merge-seqs \
  --i-data data/processed/emp_data/tapwater_seqs.qza \
  --i-data data/processed/emp_data/90bp_decontam_concrete_seqs.qza \
  --i-data data/processed/emp_data/emp_deblur_90bp.release1_seqs.qza \
