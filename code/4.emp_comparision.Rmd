...
inds %>% 
  write.xlsx(file="g:/Shared drives/Maresca Lab Shared Stuff/Concrete/Cylinder series paper/Figures/final/ST11_indicator_values.xlsx", sheetName="ASV", append=TRUE, row.names=FALSE)
```
...
```{r}
metadata <- read_delim("data/processed/emp_data/emp_and_tapwater_comparison_map.tsv", "\t", escape_double = FALSE, trim_ws = TRUE) %>%
  mutate(empo_3 = if_else(empo_3 == "Concrete" & str_detect(`#SampleID`, "U"), "ASR_pos", empo_3)) %>%
  mutate(empo_3 = if_else(empo_3 == "Concrete" & str_detect(`#SampleID`, "M"), "ASR_neg", empo_3))
```

#### Indicator **genera** analysis, with ASR +/- concrete split

```{bash ind. genera analysis}
date
ssh akiledal@biomix.dbi.udel.edu "sbatch" <<'ENDSSH'
#!/usr/local/bin/Rscript
#SBATCH --job-name=g_inds_concrete_asr
#SBATCH --chdir=/home/akiledal/Concrete_analysis
#SBATCH --mem=500G
#SBATCH -c 2
#SBATCH --mail-type=ALL
...
#Filter for taxa observed at least 10 times in 5 samples, and remove precursors
ps <- raw_series_phyloseq %>% 
  filter_taxa(function(x) sum(x > 10) > 5, TRUE) %>% 
  subset_samples(!empo_3 %in% c("FlyAsh","Gravel","Powder","Sand", "Glass"))
...
ind_val <- multipatt(otu_table,groupings, max.order = 2, control = how(nperm = 999))
...
saveRDS(ind_val,"data/processed/emp_data/asr_g_concrete_indicators.rds")
saveRDS(ind_ci, "data/processed/emp_data/asr_g_concrete_ind_ci.rds")
...
concrete_genus_inds <- inds %>% 
  ...
  write.xlsx(file="g:/Shared drives/Maresca Lab Shared Stuff/Concrete/Cylinder series paper/Figures/final/ST11_indicator_values.xlsx", sheetName="genus", append=TRUE, row.names=FALSE)
...
(decontam_change <- combined_similarity %>%
  ...
  ggsave("g:/Shared drives/Maresca Lab Shared Stuff/Concrete/Cylinder series paper/Figures/final/SF4_emp_decontam_similarity_change.png", type = "cairo"))
