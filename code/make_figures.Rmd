...
## Figure 4: Influence of precursors on concrete bacterial communities
...
unmit_unknown + mit_unknown + plot_layout(guides = 'collect') + plot_annotation(tag_levels = "a") + 
  ggsave("g:/Shared drives/Maresca Lab Shared Stuff/Concrete/Cylinder series paper/Figures/final/Fig4_SourceTracker_precursors.pdf", 
         device = cairo_pdf, dpi = 600, width = 6, height = 4) +
  ggsave("g:/Shared drives/Maresca Lab Shared Stuff/Concrete/Cylinder series paper/Figures/final/Fig4_SourceTracker_precursors.eps", 
         device=cairo_ps, fallback_resolution = 600, dpi = 600, width = 6, height = 4) +
  ggsave("g:/Shared drives/Maresca Lab Shared Stuff/Concrete/Cylinder series paper/Figures/final/Fig4_SourceTracker_precursors.png", 
         type = "cairo", dpi = 600, width = 6, height = 4)
```

## Figure 5: Within-sample diversity over time.
...
shannon_tab <- tableGrob(summary(shannon_lme)$coefficients %>% 
                         ...
                         mutate(p.value = if_else(p.value < 0.05, paste0(p.value,"**"),paste(p.value))), rows = NULL, theme = ttheme_default(base_size = 9))
...
wrap_plots(shannon_tab, shannon, shan_precursors, faith_tab, faith, faith_precursors) + 
  ...
  plot_annotation(tag_levels = 'a') + 
  ggsave("g:/Shared drives/Maresca Lab Shared Stuff/Concrete/Cylinder series paper/Figures/final/Fig5_alpha_diversity_decontam.pdf", 
         device = cairo_pdf, dpi = 600, width = 6.8, height = 4, scale = 1.5) +
  ggsave("g:/Shared drives/Maresca Lab Shared Stuff/Concrete/Cylinder series paper/Figures/final/Fig5_alpha_diversity_decontam.eps", 
         device=cairo_ps, fallback_resolution = 600, dpi = 600, width = 6.8, height = 4, scale = 1.5) +
  ggsave("g:/Shared drives/Maresca Lab Shared Stuff/Concrete/Cylinder series paper/Figures/final/Fig5_alpha_diversity_decontam.png", 
...
permanova_table <- bind_rows(seq_summary,margin_summary) %>% 
  ...
  left_join(disp) %>% 
  mutate(across(c("F","p.value","disp_p_val"),~round(.,3))) %>% 
...
symbols <- data.frame(tax = multiple) %>% 
  mutate(ind = if_else(tax %in% indic_sp, "*",""),
         meta = if_else(tax %in% metaSeq, "#",""),
         log = if_else(tax %in% logit, "^",""),
         symbols = glue::glue("{ind}{meta}{log}")) %>% 
  rename(og_taxa = "tax")
...
mult_df <- mult$data %>% 
  left_join(symbols) %>% 
  mutate(tax_and_level = glue::glue("{tax_and_level}{symbols}"))
...
## ST2: Contaminant taxa

Produced in the decontamination .Rmd document.

## ST3: Contaminants by detection method
...
## SF4: Decontamination changes in concrete similarity to EMP groups
...
## SF6: Within sample diverity before decontamination
...
wrap_plots(shannon_tab, shannon, shan_precursors, faith_tab, faith, faith_precursors) + 
  plot_layout(design = design, guides = "collect",heights = c(1,1,0.3)) + 
  plot_annotation(tag_levels = 'a') + 
  ggsave("g:/Shared drives/Maresca Lab Shared Stuff/Concrete/Cylinder series paper/Figures/final/SF6_alpha_diversity_raw.pdf", 
         device = cairo_pdf, dpi = 600, width = 6.8, height = 4, scale = 1.5) +
  ggsave("g:/Shared drives/Maresca Lab Shared Stuff/Concrete/Cylinder series paper/Figures/final/SF6_alpha_diversity_raw.eps", 
         device=cairo_ps, fallback_resolution = 600, dpi = 600, width = 6.8, height = 4, scale = 1.5) +
  ggsave("g:/Shared drives/Maresca Lab Shared Stuff/Concrete/Cylinder series paper/Figures/final/SF6_alpha_diversity_raw.png", 
         type = "cairo", dpi = 600, width = 6.8, height = 4, scale = 1.5)
...
## SF7: Concrete similarity to EMP groups over time
...
decontam_plot_data %>%
  ...
  theme_bw() +
  ggsave("g:/Shared drives/Maresca Lab Shared Stuff/Concrete/Cylinder series paper/Figures/final/SF7_concrete_EMPO3_sim_over_time.png", type = "cairo", width = 8, height = 6)
...
## ST9: Change in relative abundance over time

Computed in the code below alongside ST10.

## ST10:: Seasonal changes in relative abundance
...
seasonal_wide <- seasonal_taxa %>%
  ...
  write_tsv("g:/Shared drives/Maresca Lab Shared Stuff/Concrete/Cylinder series paper/Figures/final/ST9_long-term_relative_abundance_change.tsv")

over_time_wide <- over_time_taxa %>%
  pivot_wider(names_from = term ,values_from = c(edf,ref.df,statistic,p.value)) %>% 
  write_tsv("g:/Shared drives/Maresca Lab Shared Stuff/Concrete/Cylinder series paper/Figures/final/ST10_seasonal_relative_abundance_change.tsv")
